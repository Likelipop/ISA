...
\begin{frame}{Methodology: Generalized Permutation Test}
    ...
    \begin{enumerate}
        ...
        Combine all observations ($Y_1, \ldots, Y_n$) under the null hypothesis.
        \vfill
...
\section{Permutation Test result}

\begin{frame}[fragile]{Execution and Results (R = 10,000)}
    \begin{block}{Output}
        \scriptsize
\begin{verbatim}
...
\begin{frame}
    \begin{figure}
        \centering
        \includegraphics[width=0.7\linewidth]{images/permutation_mean.jpeg}
        \caption{Permutation Distribution: Mean Time Difference.}
    \end{figure}
\end{frame}
...
\begin{frame}
    \begin{figure}
        \centering
        \includegraphics[width=0.7\linewidth]{images/permutation_conver_rate.jpeg}
...
\begin{frame}{Interpretation and Conclusion}
    \begin{block}{Analysis 1: Time-on-Page (H1: new > old)}
        \begin{itemize}
            ...
            \item Since the p-value ($0.0001$) is less than our significance level ($\alpha = 0.05$), we \textbf{reject the null hypothesis ($H_0$)}.
    ...
    \begin{block}{Analysis 2: Conversion Rate (H1: new > old)}
        \begin{itemize}
            ...
            \item Since the p-value ($0.0147$) is less than our significance level ($\alpha = 0.05$), we \textbf{reject the null hypothesis ($H_0$)}.
...
\begin{frame}{Limitations of Permutation Tests}
    \begin{itemize}
        ...
        \vfill
        \item The only core assumption is the \textbf{exchangeability} of observations under the null hypothesis ($H_0$).
    \end{itemize}
\end{frame}
